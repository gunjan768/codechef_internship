<?php
	...
	class Repository
	{
	    ...
	    public function getContestDetails($token, $contestCode)
	    {
	        ...
	        if(empty($contest) || empty($contest->banner) || empty($problems) || Utils::shouldUpdateCache($problems[0]->lastUpdated)) 
	        {
	            ...
	            if(empty($problems)) 
	            {
	                ...
	                {
	                    $problemsWriteStmt->execute(
	                    	[$item->problemCode, $contestCode, $item->successfulSubmissions, $item->accuracy, Carbon::now()]
	                    );
	                }
	            }
	    ...
	    {
	        $probSubmissionsStmt = $this->conn->prepare(
	        	"SELECT * FROM problemSubmissions WHERE contestCode = ? AND problemCode = ? ORDER BY date DESC");
